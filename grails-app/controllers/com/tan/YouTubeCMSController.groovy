...
class YouTubeCMSController {

    def index() {}
    def getChannel={
        def channelList=TblYoutubeChannel.findAll();
        render channelList as JSON;
    ...
    def cmsChannel={
        ...
        //String contentOwnerID="MbsnpwkyzCywRCewfNJQJA";
        String contentOwnerID=params?.channel_list;
        ...
        def ContentOwner=TblYoutubeCMS.findByContentOwnerID(contentOwnerID);
        def result=TblCMS.findAllByContentOwnerIDAndStartDateBetween(ContentOwner,startDate,endDate);
        if (params?.filterViews=="daily"){
        ...
        if (params?.filterViews == "monthly"){
            ...
            def month_map = [:];
            def resultData=TblCMS.findAllByContentOwnerIDAndEndDateBetween(ContentOwner,start_Date,end_Date);
        ...
        /** customize channel selection the code is use ful l****/

        /*def selectedChannel = params.channel_list.split(',');
        def selectedChannelCopy = [];
        if (selectedChannel.size() > 1) {
            selectedChannel.collect(selectedChannelCopy) {
                it.toString()
            }
            println(selectedChannel);
            println(selectedChannelCopy);
       ...
       *//* def channelList=TblYoutubeChannel.findAll();*//*
        ...
        long favoritesAddedCount = 0;
        long sharesCount = 0;
        long subscriberGAINEDCount = 0;
        long subscriberLOSTCount = 0;
        long watchTimeCount = 0;
        ...
        def dateInDaily="";
            ...
            startDate = startCal.get(Calendar.YEAR)+"-"+startMonth+"-"+startDay;
            endDate=     endCal.get(Calendar.YEAR)+"-"+endMonth+"-"+endDay;
            def channelList=TblYoutubeChannel.findAllByChannelIdInList(selectedChannelCopy);
            def resultList=TblViews.findAllByChannelIdInListAndStartDateBetween(channelList,startDate,endDate);
        ...
        if (params?.filterViews=="daily"){
            ...
            for (date in toDate..fromDate) {
                ...
                dailyMap.put("views", viewCount);
                dailyMap.put("likes", likeCount);
                dailyMap.put("dislikes", disLikeCount);
                dailyMap.put("comments", commentsCount);
                dailyMap.put("favoritesAdded", favoritesAddedCount);
                dailyMap.put("shares", sharesCount);
                dailyMap.put("subscriberGAINED", subscriberGAINEDCount);
                dailyMap.put("subscriberLOST", subscriberLOSTCount);
                dailyMap.put("watchTime", watchTimeCount);
                dailyMap.put("aDVERTISING", advertisingCount);
                dailyMap.put("endDate", date_of_week);
                final_daily_map.put(date_of_week, dailyMap);
                viewCount = 0;
                likeCount = 0;
                disLikeCount = 0;
                commentsCount = 0;
            ...
            println(final_daily_map);
            render final_daily_map as JSON;
        }
        ...
        if (params?.filterViews == "weekly") {
            ...
            for (date in toDate..fromDate) {
                ...
                resultList.each { e ->
                    if (e.endDate.equals(date_of_week)) {
                        start_date = dateInstance.getAt(Calendar.DAY_OF_WEEK);
                        if (start_date == 1 || start_date == 2 || start_date == 3 || start_date == 4 || start_date == 5 || start_date == 6 || start_date == 7) {

                            println("date_of_week    1::>" + date_of_week);
                            viewCount = viewCount + e?.views;
                            likeCount = likeCount + e?.likes;
                            disLikeCount = disLikeCount + e?.disLikes;
                            commentsCount = commentsCount + e?.comments;
        ...
        if (params?.filterViews == "monthly"){
            ...
            start_Date = toInstance.get(Calendar.YEAR)+"-"+start_Month+"-"+start_Day;
            end_Date = fromInstance.get(Calendar.YEAR)+"-"+end_Month+"-"+end_Day;
            String start_Month_year = toInstance.get(Calendar.YEAR)+"-"+start_Month;
            String end_Month_year = fromInstance.get(Calendar.YEAR)+"-"+end_Month;
            def month_map = [:];
            //def resultData=TblViews.findAllByChannelIdAndEndDateBetween(channel,start_Date,end_Date);
            def resultData=TblViews.findAllByChannelIdInListAndEndDateBetween(channelList,start_Date,end_Date);
            println(">>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>"+start_Date+">>>>>>>>>>>>>>>>>>>>>>"+end_Date);
            SimpleDateFormat dateFormatter = new SimpleDateFormat("yyyy-MM-dd");
            Calendar calendarInstance = Calendar.instance
            Date date_start= dateFormatter.parse(start_Date);
            Date date_end= dateFormatter.parse(end_Date);

            Calendar startCalendar = new GregorianCalendar();
            startCalendar.setTime(date_start);
            Calendar endCalendar = new GregorianCalendar();
            endCalendar.setTime(date_end);
            int diffYear = endCalendar.get(Calendar.YEAR) - startCalendar.get(Calendar.YEAR);
            int diffMonth = diffYear * 12 + endCalendar.get(Calendar.MONTH) - startCalendar.get(Calendar.MONTH);

            int nextDate=startingMonth;
            for(int month_var=0; month_var<=diffMonth;month_var++){
                println(">>>>>>>>>>>"+month_var);
                viewCount = 0;
                likeCount = 0;
                disLikeCount = 0;
                ...
                resultData.each{ e->
                    ...
                    if (calender_Month==nextDate) {
                        ...
                        watchTimeCount = watchTimeCount + e.watchTime;
                        advertisingCount = advertisingCount + e.aDVERTISING;
                        nextDate = calender_Month;
                        mapDate =  startDateString;
                    }
                    else{
                        return true;
                    }
                    if (endingMonth==calender_Month){
                        return true;
                    }
                }
                month_map.put("views", viewCount);
                ...
                month_map.put("aDVERTISING", advertisingCount);
                month_map.put("endDate", monthString);
                final_month_map.put(mapDate,month_map);
            }
            render final_month_map as JSON;
        }
*/
