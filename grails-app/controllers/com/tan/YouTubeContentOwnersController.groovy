...
class YouTubeContentOwnersController {
     ...
     */
    private  String contentOwner_id = "MbsnpwkyzCywRCewfNJQJA";
    ...
    def contentOwnersReport={

        try{
        /*String date_from = params?.date_from;
        String date_to =   params?.date_to;*/

        /*String date_to =   "18-12-2013";
        String date_from = "21-02-2015";*/
        ...
        analytics = new YouTubeAnalytics.Builder(HTTP_TRANSPORT, JSON_FACTORY, credential)
           ...
           // Date fromDate = formatter.parse(date_from);
            ...
            for (date in toDate..dateEnd) {
                ...
                if(firstCount==0){
                    def past_list=TblCMS.findByContentOwnerIDAndEndDate(TblYoutubeCMS.findByContentOwnerID(contentOwner_id),endDate);
                    if (past_list){
                ...
                if (startDate != "") {
                   ...
                   /* ResultTable resultList= analytics.reports().query("contentOwner=="+contentOwner_id,"2014-05-01","2014-06-30","views,comments,favoritesAdded,likes,dislikes,estimatedMinutesWatched,averageViewDuration").setFilters("claimedStatus==claimed").execute();*/

                   /* println(">>>>>>>>>>>"+resultList);*/


                    ResultTable results = analytics.reports().query("contentOwner=="+contentOwner_id, startDate, endDate, "views").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultEstimateWatchedTime= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"estimatedMinutesWatched").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultLikes= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"likes").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultDislikes= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"dislikes").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultShares= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"shares").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultSubscribersGained= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"subscribersGained").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultSubscribersLost= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"subscribersLost").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultComments= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"comments").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultFavoritesAdded= analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"favoritesAdded").setFilters("claimedStatus==claimed").execute();
                    ResultTable resultTrafficSource = analytics.reports().query("contentOwner=="+contentOwner_id,startDate,endDate,"views").setDimensions("insightTrafficSourceType").setFilters("claimedStatus==claimed").execute();
                    ...
                    new TblCMS(
                            ...
                            aNNOTATION: ANNOTATION,
                            aDVERTISING: ADVERTISING,
                            contentOwnerID: TblYoutubeCMS.findByContentOwnerID(contentOwner_id),
                    ).save(failOnError: true);
...
/*           ResultTable results= analytics.reports()
           ...
           /* render axis as JSON;*/
            redirect(controller:'addCMS',action: 'cms');
